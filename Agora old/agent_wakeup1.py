...
def create_response(model: str, messages: List[dict], temperature: float, max_output_tokens: int) -> str:
    ...
    NOTE:
    - Enforces API minimum: max_output_tokens >= 16
    """
    ...
    if client is not None:
        ...
        # SDKs may expose either .output_text or a nested structure.
    ...
    # HTTP fallback (requires requests)
    payload = {
        "model": model,
        "input": messages,
        "temperature": temperature,
...
class ChatAgent:
    ...
    def system_prompt(self, scene: str, all_names: List[str]) -> str:
        return (
            f"You are {self.name} in a group chat with {', '.join(all_names)} and a user.\n"
            f"You are one of several FRIENDS chatting together; you can talk to each other naturally.\n"
            f"Stay in character and follow the ROLE INSTRUCTIONS below.\n"
...
# -------------------------------
# Admin prompts (fixed in script) - ENGLISH
# -------------------------------
...
def main():
    ...
    # Enforce API minimum for user-configured arg too
    ...
    # Load scene + role files (fallback placeholders if missing)
    scene = safe_read_text(args.scene, default="(scene file missing)")
    role_a = safe_read_text(args.bot1, default="(chatbot1.txt missing)")
    role_b = safe_read_text(args.bot2, default="(chatbot2.txt missing; placeholder role) You are Agent 2: the Calm & Analytical Friend.")
    role_c = safe_read_text(args.bot3, default="(chatbot3.txt missing; placeholder role) You are Agent 3: the Skeptical & Funny Friend.")
    ...
    with open(chat_path, "a", encoding="utf-8") as chat_fp, open(thinking_path, "a", encoding="utf-8") as thinking_fp:
        ...
        def agent_turn(agent: ChatAgent, force_intro: bool = False):
            ...
            user_prompt = (
                "Here is the full group chat transcript so far (newest near bottom). "
                "Continue the conversation as your character.\n\n"
                f"{history}\n{extra}"
            )

            messages = [
                {"role": "system", "content": agent.system_prompt(scene, all_names)},
        ...
        def admin_choose_next() -> str:
            """Returns 'A'/'B'/'C'/'U'."""
            # Force user after 5 consecutive agent turns
            ...
            # Admin2 strict selector (must be >=16 tokens per API)
            admin2_messages = [
