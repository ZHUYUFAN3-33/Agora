...
class ChatAgent:
    ...
    def system_prompt(self, scene: str, name_map: Dict[str, str]) -> str:
        ...
        return (
            ...
            f"- U: user\n\n"
            f"GROUP DYNAMICS (guidance):\n"
            f"- This is a FRIEND group chat. You can sometimes talk to other bots, not only the user.\n"
            f"- Keep it natural and not too frequent: inter-bot questions are occasional, not every message.\n"
...
PACING GOAL:
- Prefer A/B/C speaking over the user, as long as it still feels coherent.
- Encourage some bot-to-bot interaction, but keep it occasional and natural (not every message).
...
def main():
    ...
    ap.add_argument("--max_user_gap", type=int, default=12,
                    help="If user hasn't spoken in this many transcript lines, force U (unless 5-agent rule forces U first). Default 12")

    # NEW: lower inter-bot nudge frequency
    ap.add_argument("--interact_rate", type=float, default=0.35,
                    help="Probability to add an extra nudge encouraging bot-to-bot interaction in a given agent turn (0..1). Default 0.35")

    args = ap.parse_args()
    args.max_output_tokens = max(int(args.max_output_tokens), MIN_OUTPUT_TOKENS)
    args.prefer_agents = max(0.0, min(1.0, float(args.prefer_agents)))
    args.interact_rate = max(0.0, min(1.0, float(args.interact_rate)))
    ...
    with open(chat_path, "a", encoding="utf-8") as chat_fp, open(thinking_path, "a", encoding="utf-8") as thinking_fp:
        ...
        def agent_turn(agent: ChatAgent, force_intro: bool = False):
            ...
            # Occasional nudge (NOT every time)
            nudge = ""
            if random.random() < args.interact_rate:
                nudge = (
                    "\n\n(Soft nudge) If it feels natural, briefly react to another bot or ask ONE short question to one bot. "
                    "Do not force it if it feels awkward."
                )
            ...
            user_prompt = (
                ...
                "Inter-bot interaction should be occasional and natural (not every message).\n\n"
                f"{history}{nudge}\n{extra}"
            )
        ...
        def admin_choose_next() -> str:
            ...
            gap = len(transcript_lines) if li is None else (len(transcript_lines) - 1) - li
